    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        try {
            ...
            if (!this.isLogined(httpRequest, session)) {
                ...
                Cookie cookiesURL = new Cookie(SessionConstant.ACCESS_URL, url);
                cookiesURL.setPath("/");
                // - value mean the cookie will be delete when browse closed.
                cookiesURL.setMaxAge(-1);
                httpResponse.addCookie(cookiesURL);

                String ssourl = filterConfig.getInitParameter(SessionConstant.PASSPORT_URL);
        ...
        try {
            ...
            if (cookies != null) {
                for (Cookie cookie : cookies) {
                    if (cookie.getName().equals(SessionConstant.LOGIN_ID)) {
                        session.setAttribute(SessionConstant.LOGIN_ID, cookie.getValue());
                        result = true;
                    } else if (cookie.getName().equals(SessionConstant.USER_ID)) {
                        session.setAttribute(SessionConstant.USER_ID, Integer.parseInt(cookie.getValue()));
                    } else if (cookie.getName().equals(SessionConstant.USER_NAME)) {
                        String userName = URLDecoder.decode(cookie.getValue(), SessionConstant.COOKIE_ENCODING_UTF8);
